...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='715fef99d38cc07ac5536f440cf6d7251b63ed7d'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'tag: v0.4.2, release-0.4' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'tag: v0.4.2, release-0.4' =~ tag:\ (v[^ ,]+) ]]; then
