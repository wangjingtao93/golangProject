...
import (
	...
	"kubernetes-csi-tencentcloud/driver/cfs"
)
...
var (
	endpoint = flag.String("endpoint", fmt.Sprintf("unix://plugin/%s/csi.sock", cfs.DriverName), "CSI endpoint")
	region   = flag.String("region", "sz", "tencent cloud api region")
	zone     = flag.String("zone", "ns", "cvm instance region")
	cfsUrl   = flag.String("cfs_url", "cfs.internal.tencentcloudapi.com", "cfs api domain")
	nodeID   = flag.String("nodeID", "deploy-node", "node ID")
)
